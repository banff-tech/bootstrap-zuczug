...
//				String itemIssuanceId = (String) itemIssuanceCreate.get("itemIssuanceId");
	...
	public static String importQuoteFromExcel(LocalDispatcher dispatcher, Delegator delegator,GenericValue userLogin,List<Map<String,String>> skuItemList,String customerId,String productStoreId) throws GenericEntityException, GenericServiceException{
		String errorMessage = "";
		String facilityId = "";
		if(skuItemList!=null&&!skuItemList.isEmpty()){
			GenericValue buyerProductStore = delegator.findByPrimaryKey("ProductStore", UtilMisc.toMap("productStoreId", customerId));
			if (buyerProductStore != null) {
				facilityId = buyerProductStore.getString("inventoryFacilityId");
			} else {
				errorMessage="买家收货地场所为空";
				return "error";
			}
			Map<String, Object> createShipmentInfo = dispatcher.runSync("createShipment", UtilMisc.toMap("shipmentTypeId", "TRANSFER","statusId","SHIPMENT_INPUT",
					"originFacilityId","ZUCZUG_CLOTHESFACILITY","destinationFacilityId",facilityId,"currencyUomId","CNY","userLogin", userLogin));
			for(Map<String,String> skuItem:skuItemList){
				String productId = skuItem.get("productId");
				String quantity = skuItem.get("quantity");
				Map<String, Object> createShipmentItemInfo = dispatcher.runSync("createShipmentItem", UtilMisc.<String, Object>toMap("shipmentId", createShipmentInfo.get("shipmentId"),"productId",productId,
						"quantity", new BigDecimal(quantity),"userLogin", userLogin));
			}
		}
		return errorMessage;
	}
