          ...
          <tr>
            ...
            <td class="button-col">
              ...
              <#if security.hasEntityPermission("FACILITY", "_UPDATE", session)>
                <a href='<@ofbizUrl>EditContactMech?facilityId=${facilityId}&amp;contactMechId=${contactMech.contactMechId}</@ofbizUrl>'>修改</a>
              </#if>
              <#if security.hasEntityPermission("FACILITY", "_DELETE", session)>
                <form action="<@ofbizUrl>deleteFacilityContactMechPurpose/ViewContactMechs</@ofbizUrl>" name="deleteContactForm_${contactMechMap_index}" method="post">
                  <input type="hidden" name="facilityId" value="${facilityId?if_exists}"/>
                  <input type="hidden" name="contactMechId" value="${contactMech.contactMechId?if_exists}"/>
                  <#if contactMechPurposeType?has_content>
	                  <#assign contactMechPurposeTypeId = contactMechPurposeType.get("contactMechPurposeTypeId",locale)/>
					  <input type="hidden" name="contactMechPurposeTypeId" value="${contactMechPurposeTypeId}"/>
				  </#if>
                  <input type="hidden" name="fromDate" value="${facilityContactMech.fromDate.toString()?if_exists}"/>
                </form>
                <br>
