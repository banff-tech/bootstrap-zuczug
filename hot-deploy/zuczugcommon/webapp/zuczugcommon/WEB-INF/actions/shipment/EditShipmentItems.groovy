...
if (shipment) {
    shipmentItems = shipment.getRelated("ShipmentItem", null, ['shipmentItemSeqId DESC']);
    ...
    if (shipmentItems) {
        shipmentItems.each { shipmentItem ->
			...
			totalRequirementQuantity = 0;
            requirementGroupbyProduct = EntityUtil.getFirst(delegator.findByAnd("RequirementGroupbyProductId", UtilMisc.toMap("productId",shipmentItem.productId,"facilityId",shipment.destinationFacilityId)));
            if (requirementGroupbyProduct?.quantity != null) {
                totalRequirementQuantity = requirementGroupbyProduct.quantity;
            }
            
			totalInventoryQuantity = 0;
            inventoryItemGroupbyProduct = EntityUtil.getFirst(delegator.findByAnd("InventoryItemGroupbyProductId", UtilMisc.toMap("productId",shipmentItem.productId,"facilityId",shipment.originFacilityId)));
            if (inventoryItemGroupbyProduct?.availableToPromiseTotal != null) {
                totalInventoryQuantity = inventoryItemGroupbyProduct.availableToPromiseTotal;
            }
            ...
            shipmentItemData.totalInventoryQuantity = totalInventoryQuantity;
            shipmentItemData.totalRequirementQuantity = totalRequirementQuantity;
